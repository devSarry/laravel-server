...
if [[ "$1" && "$2"  ]]
	then
		...
		if [ -f /home/capybara/.mongo ]
		then
		    echo "MongoDB already installed."
		    exit 0
		fi

		touch /home/capybara/.mongo
		...
		mongo admin --eval "db.createUser({user:'\`$mongo_user\`',pwd:'\`$mongo_password\`',roles:['root']})"
	else
	    echo "Error: missing one ore more of  the three required parameters."
	    echo "Usage: bash install-mongo.sh mongo_user mongo_password"
	fi